...
export const post = defineType({
  ...
  fields: [
    ...
    defineField({ name: "excerpt", type: "localeString", title: "Resumo", options: { canvasApp: { purpose: "Resumo curto (1–2 frases) para cards e listagens do blog — não é o corpo completo." } } }),
    defineField({ name: "body", type: "localeString", title: "Corpo (Markdown)" }),
    defineField({ name: "cat", type: "localeString", title: "Categoria", options: { canvasApp: { purpose: "Categoria única do artigo (ex.: Regulação, Mercado, Eventos, Posições, Inteligência)." } } }),
